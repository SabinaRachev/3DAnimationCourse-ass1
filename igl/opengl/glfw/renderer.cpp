...
void Renderer::MouseProcessing(int button)
{
	...
	if (scn->isPicked )
	{
		...
		{
			scn->data().RotateInSystem(Eigen::Vector3d(1, 0, 0), yrel / 100);;

			scn->data().RotateInSystem(Eigen::Vector3d(0, 1, 0), xrel / 100);
	...
	else
	{
		...
		{
			scn->RotateInSystem(Eigen::Vector3d(1,0,0),yrel/100);
			scn->RotateInSystem(Eigen::Vector3d(0, 1, 0), xrel / 100);;
